<?php

function sync_branding_price_table_creation()
{
    $curl = curl_init();

    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://vendorapi.amrod.co.za/api/v1/BrandingPrices/',
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => '',
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 0,
      CURLOPT_FOLLOWLOCATION => true,
      CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
      CURLOPT_CUSTOMREQUEST => 'GET',
      CURLOPT_HTTPHEADER => array(
        'Authorization: Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6IjMwRTZGNTY1QkUzRTkyNTRFOUREMDcwQzhCQzYwQkYwRjI1RkE5NDhSUzI1NiIsInR5cCI6ImF0K2p3dCIsIng1dCI6Ik1PYjFaYjQta2xUcDNRY01pOFlMOFBKZnFVZyJ9..RVUdH_g1lZJlmltLIoKHBaKU76QiYCYO_A4pMs6eGQs7ZcHoUjavhtwUvJxA063YXoauYEtnkktXZn6UL66YEQ74omc0y1NOMrnoVmss1o4Uf4nkqTHK9UJDiZza9Xme1EVM1FMx6RWhmGHHJ3X3relwp0qf2WdYbgm3TEkqYRVLl6TFDRCFaO3Tc2mTM-jSmD2hGqreIk8PY5ookUyT0zRoEij9IHNiYhmC2pFCcIK8qzFsi2DScf2CpnaScIqdzccv49bJa4L5aFDwoQTPPzEShFIMN4KXILW4DZcXlEk78bKpfyygFR6sz3Nl_hqKqH-XlYJetmUhObc9RKJTAw',
        'Cookie: SERVERID=iis2; __cflb=02DiuEfjaS5QYNRiHiy4sYLgVKJh4ozFrhwoQUbQvrb9N'
      ),
    ));
    
    $response = curl_exec($curl);
    
    curl_close($curl);
    return $response;
}

function get_branding_price_to_callback()
{

    ob_start();
    $api_response = sync_branding_price_table_creation();
    $branding_price = json_decode($api_response, true);
    // Insert to database
    global $wpdb;
    $table_name = $wpdb->prefix . 'sync_branding_price';
    $wpdb->query("TRUNCATE TABLE $table_name");

    foreach ($branding_price as $branding) {
        $branding_json = json_encode($branding);
        $wpdb->insert(
            $table_name,
            [
                'operation_type'  => 'branding_price_create',
                'operation_value' => $branding_json,
            ]
        );
    }

    echo '<h4>Stock inserted successfully</h4>';

    return ob_get_clean();
}

add_shortcode('get_brandign_price', 'get_branding_price_to_callback');
